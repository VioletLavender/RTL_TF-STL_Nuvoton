...
RT568FT_TestStatus RT568_DeepSleepTest(void)
{
    ...
    // set reset pin to input pin to avoid leakage current in sleep mode
    MCU_SetGpioResetIdle();
...
RT568FT_TestStatus RT568_RssiDUT_RxTest(int8_t rssiBase, uint8_t rssiRange, signed char* rssi)
{
    ...
    // check rssi
    Tiny_Delay(500);
    *rssi = RT568_GetRssiValue();

    if(RT568_CheckRssiValue(*rssi,rssiBase,rssiRange) == 0)
    {
...
static void RT568_SetRssiTestRx(void)
{
    ...
    Tiny_Delay(5000);           // MUST to added the delay
