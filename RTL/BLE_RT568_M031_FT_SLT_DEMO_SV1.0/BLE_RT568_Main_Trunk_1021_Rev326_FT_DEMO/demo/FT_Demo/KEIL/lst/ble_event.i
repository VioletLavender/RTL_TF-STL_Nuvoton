...
#line 19 "..\\..\\..\\source\\ble_event.c"
#line 1 "..\\..\\..\\include\\ble_event.h"
...
#line 21 "..\\..\\..\\source\\ble_event.c"
#line 22 "..\\..\\..\\source\\ble_event.c"
#line 23 "..\\..\\..\\source\\ble_event.c"

BleEventCallBack bleEventCallBack; 


static void MHC_Callback(BleCmdEvent event, void* param);
...
void setBLE_RegisterBleEvent(BleEventCallBack callback)
{
    extern void initLLCallBack(MHCCallBack callback);
    initLLCallBack(MHC_Callback);  
...
static void BleEvent_PostEvent( BleCmdEvent event, void *param)
{
    bleEventCallBack(event, param);
}
...
static void MHC_Callback(BleCmdEvent event, void* param)
{
    ...
    {
    case 0x01:
        BleEvent_PostEvent(0x01, (BLE_Event_ADV_Complete_Param *)param);
        break;

    case 0x0E:
        BleEvent_PostEvent(0x0E, 0);
        break;

    case 0x02:
    {
        struct MHC_Le_Adv_Report_Para *report_param = (struct MHC_Le_Adv_Report_Para *)param;
        BLE_Event_Scan_Report evt;
        Knl_MemCpy(&evt.Rpt_Type, &report_param->HCI_Event_Type0, report_param->HCI_Length0+9);
        evt.Rpt_Rssi = report_param->HCI_Data0[report_param->HCI_Length0];
        BleEvent_PostEvent(0x02, (BLE_Event_Scan_Report *)&evt);
    }
    break;

    case 0x03:
    {
        struct MHC_Le_Conn_Complete_Para *connparam = (struct MHC_Le_Conn_Complete_Para *)param;
        BLE_Event_ConnParam eventParam;

        eventParam.status = connparam->HCI_Status;
        eventParam.connId = connparam->HCI_Conn_Hdl_L;
        eventParam.connInterval = ((Uint16)(connparam->HCI_Conn_IntervalH) << 8 )|(connparam->HCI_Conn_IntervalL);
        eventParam.connLatency = ((Uint16)(connparam->HCI_ConnLatencyH) << 8 )|(connparam->HCI_ConnLatencyL);
        eventParam.connRole = connparam->HCI_Role;
        eventParam.supervisionTimeout = ((Uint16)(connparam->HCI_SvisionTimeoutH) << 8 )|(connparam->HCI_SvisionTimeoutL);
        eventParam.peerAddr.addrType = connparam->HCI_PeerAddrType;
        Knl_CodeCpy(eventParam.peerAddr.addr,connparam->HCI_PeerAddr, 6);

        BleEvent_PostEvent(0x03,(BLE_Event_ConnParam *)&eventParam);
    }
    break;

    case 0x07:
    {
        struct MHC_Disconn_Complete_Para *connparam = (struct MHC_Disconn_Complete_Para *)param;
        BLE_Event_DisconnParam eventParam;

        eventParam.connId = connparam->HCI_Conn_Hdl_L;
        eventParam.disconnectReason = connparam->HCI_Reason;

        BleEvent_PostEvent(0x07,(BLE_Event_DisconnParam *)&eventParam);
        clearBLE_CmdTimer(eventParam.connId, 0xFF); 
    }
    break;

    case 0x05:
    {
        BLE_Event_ConnUpdateParam eventParam;
        eventParam.connId = *((Uint8 *)(param)+1);
        eventParam.status = *((Uint8 *)(param));

        if(eventParam.status == 0x00)
        {
            if(getBLE_CmdTimerEvent(eventParam.connId, 0x00) == 0x00)
            {
                clearBLE_CmdTimer(eventParam.connId, 0x00);
                setBLE_CmdTimer(eventParam.connId, 0x01, 480000); 
            }
        }
        else
        {
            clearBLE_CmdTimer(eventParam.connId, 0x00);
            BleEvent_PostEvent(0x06,(BLE_Event_ConnUpdateParam *)&eventParam);
        }
    }
    break;
    ...
    case 0x06:
    {
        struct MHC_Le_Conn_Update_Complete_Para *connparam = (struct MHC_Le_Conn_Update_Complete_Para *)param;
        BLE_Event_ConnUpdateParam eventParam;

        eventParam.connId = connparam->HCI_Conn_Hdl_L;
        eventParam.status = connparam->HCI_Status;
        eventParam.connInterval = ((Uint16)(connparam->HCI_ConnIntervalH) << 8 )|(connparam->HCI_ConnIntervalL);
        eventParam.connLatency = ((Uint16)(connparam->HCI_ConnLatencyH) << 8 )|(connparam->HCI_ConnLatencyL);
        eventParam.supervisionTimeout = ((Uint16)(connparam->HCI_SvisionTimeoutH) << 8 )|(connparam->HCI_SvisionTimeoutL);
        ...
        if(getBLE_CmdTimerEvent(eventParam.connId, 0x01) == 0x01)
        {
            clearBLE_CmdTimer(eventParam.connId, 0x01);
        }
    ...
    case 0x04:
        BleEvent_PostEvent(0x04,0);
        break;

    case 0x08:
        BleEvent_PostEvent(0x08,(BLE_Event_Phy_Update_Param *)param);
        break;

    case 0x09:
        BleEvent_PostEvent(0x09,(BLE_Event_Phy_Param *)param);
        break;

    case 0x0A:
        BleEvent_PostEvent(0x0A,(BLE_Event_Rssi_Param *)param);
        break;

    case 0x0B:
    {

        struct BLE_Event_Stk_Gen_Method *report = (struct BLE_Event_Stk_Gen_Method *)param;

        setBLE_CmdTimer(report->connId, 0x02, 240000); 

        if(report->Stk_Gen_Method != 0x00)
        {
            BleEvent_PostEvent(0x0B,(BLE_Event_Stk_Gen_Method *)param);
        }
    ...
    case 0x0C:
        ...
        BleEvent_PostEvent(0x0C,(BLE_Event_PassKey_Confirm *)param);
    ...
    case 0x0D:
    {
        ...
        struct BLE_Event_Auth_Status *report = (struct BLE_Event_Auth_Status *)param;

        BleEvent_PostEvent(0x0D,(BLE_Event_Auth_Status *)param);
        clearBLE_CmdTimer(report->connId, 0x02);

    }
    break;

    case 0x0F:
        BleEvent_PostEvent(0x0F, (BLE_Event_CreateConn_Param *)param);
        break;

    case 0x10:
        BleEvent_PostEvent(0x10, (BLE_Event_Mtu *)param);
        break;

    case 0x11:
        BleEvent_PostEvent(0x11, (BLE_Event_Data_Length_Param *)param);
        break;
