...
#line 42 "..\\..\\..\\source\\kernel.c"
#line 43 "..\\..\\..\\source\\kernel.c"
...
#line 44 "..\\..\\..\\source\\kernel.c"
#line 45 "..\\..\\..\\source\\kernel.c"
...
extern void Host_Root(Uint8 QueueId);
extern void LL_Root(Uint8 QueueId);
extern void L2CAP_Root(Uint8 QueueId);


extern void HCI_Root(Uint8 QueueId);




 
MBLK MsgBlk[((5*5+5)+6*5)];          
MQUEUE MsgQueue[5];     
MBLK *MBlk_Free;                   

Uint8 MBlk_depth_Remaining;
Uint8 MBlk_depth_Rsv;
...
void KernelInit(void)
{
    ...
    for (i=0; i<(((5*5+5)+6*5)-1); i++)
    {
        MsgBlk[i].Next = (MBLK *)&MsgBlk[i+1];
    }
    MsgBlk[i].Next = (MBLK *)0;
    MBlk_Free = (MBLK *)&MsgBlk[0];
    MBlk_depth_Remaining = ((5*5+5)+6*5);             
    MBlk_depth_Rsv = 0;                             

    for (i=0; i<5; i++)
    {
        MsgQueue[i].QIn = (MBLK *)0;
        MsgQueue[i].QOut = (MBLK *)0;
    }
}
...
MBLK *GetMsgBlk(void)
{
    ...
    {
        if(MBlk_depth_Remaining > 0)
        {
            MBlk_depth_Remaining--;
            pmblk = MBlk_Free;
            MBlk_Free = pmblk->Next;
            pmblk->Next = (MBLK *)0;
        }
        else
        {
...
MBLK *GetMsgBlk_L2_wSize(Uint8 len)
{
    MBLK *pmblk;
    MBLK *pmblk_Temp;
    ...
    else
    {
        ...
        else
        {
            ...
            if(len <= ((6+31+7)-2-1))
            {
                if(MBlk_depth_Remaining >= (num_MsgBlk+(5*5+5)))
                {
...
Uint8 CheckMsgBlk_L1_wSizeUsed(Uint8 len)
{
    ...
    __disable_irq();
    if(MBlk_depth_Remaining>=(num_MsgBlk+(5+5)))
    {
        __enable_irq();
        return 0;
    }
...
Uint8 CheckMsgBlk_L2_wSizeUsed(Uint8 len)
{
...
void SndMsgBlk(MBLK *pMsgBlk, Uint8 QueueId)
{
    MQUEUE *pqueue;

    pqueue = &MsgQueue[QueueId];
    __disable_irq();
    ...
    {
        (pqueue->QIn)->Next = (MBLK *)pMsgBlk;
        pqueue->QIn = (MBLK *)pMsgBlk;
    }
    __enable_irq();
}


void FreeMsgBlk(MBLK *pMsgBlk)
{
    __disable_irq();
    if ((pMsgBlk == (MBLK *)0) || (pMsgBlk == MBlk_Free))
    {
        __enable_irq();
        return;
    }
    pMsgBlk->Next = MBlk_Free;    
    MBlk_Free = pMsgBlk;
    MBlk_depth_Remaining++;
    __enable_irq();
}
...
void SndMsgBlks(MBLK *pMsgBlk, Uint8 QueueId)
{
    MQUEUE *pqueue;
    ...
    while((pqueue->QIn)->Next != (MBLK *)0)
    {
        pqueue->QIn = (pqueue->QIn)->Next;
    }
    __enable_irq();
}


void FreeMsgBlks(MBLK *pMsgBlk)
{
    MBLK *pmblk;
    Uint8 i;
...
MBLK *GetMsgBlk_L2_wSize_Isr(Uint8 len)
{
    MBLK *pmblk;
    MBLK *pmblk_Temp;
...
MBLK *GetMsgBlk_L1_wSize_Isr(Uint8 len)
{
    MBLK *pmblk;
    MBLK *pmblk_Temp;
...
Uint8 CheckMsgBlk_L1_wSizeUsed_Isr(Uint8 len)
{
...
Uint8 CheckMsgBlk_L2_wSizeUsed_Isr(Uint8 len)
{
    Uint16 num_MsgBlk;
    extern Uint8 LL_ConnID_Remaining;

    if(LL_ConnID_Remaining>1)
    {
        num_MsgBlk = (LL_ConnID_Remaining-1)*5;
        num_MsgBlk += MBlk_depth_Remaining;
    }
    else
    {
        num_MsgBlk = MBlk_depth_Remaining;
    ...
    if(num_MsgBlk>=(5*5+5))
    {
        num_MsgBlk = num_MsgBlk-(5*5+5);
        num_MsgBlk = num_MsgBlk*(((6+31+7)-2-1));
        if(num_MsgBlk>=len)
        {
            return 0;
        }
...
void SndMsgBlk_Isr(MBLK *pMsgBlk, Uint8 QueueId) 
{
    MQUEUE *pqueue;
...
void FreeMsgBlk_Isr(MBLK *pMsgBlk)  
{
    if ((pMsgBlk == (MBLK *)0) || (pMsgBlk == MBlk_Free))
    {
        return;
...
void SndMsgBlks_Isr(MBLK *pMsgBlk, Uint8 QueueId)
{
    MQUEUE *pqueue;
...
void FreeMsgBlks_Isr(MBLK *pMsgBlk)
{
    MBLK *pmblk;
...
MBLK *RcvMsgBlk(Uint8 QueueId)
{
    MQUEUE *pqueue;
    MBLK *pmblk;

    pqueue = (MQUEUE *)&MsgQueue[QueueId];
    __disable_irq();
    pmblk = pqueue->QOut;
...
Uint8 CheckMsgQueue(void)
{
    Uint8 queue_i;

     
    for (queue_i = 0; queue_i<5; queue_i++)
    {
        __disable_irq();
        if(MsgQueue[queue_i].QOut != (MBLK *)0)
        {
            break;
        }
        __enable_irq();
...
Uint16 EndianSwap16_Isr(Uint16 value)
{
    Uint8 i;

    i = ((Uint8 *) &value)[0];
    ((Uint8 *) &value)[0] = ((Uint8 *) &value)[1];
    ((Uint8 *) &value)[1] = i;
    return (value);
}


void EndianSwap16addr_Isr(Uint16 *value)
{
    Uint8 i;
...
void EndianSwap16addrM8_Isr(Uint16 *value)
{
    Uint16 i;

    ((Uint8 *) &i)[1] = ((Uint8 *) value)[0];
    ((Uint8 *) &i)[0] = ((Uint8 *) value)[1];
    *value = i<<3;
}
...
BleStackStatus Ble_Kernel_Root(void)
{
    BleStackStatus LLHostRun = 0;  
    Uint8 queue_id;
    extern Uint32 seedR16;
    extern void checkBLE_CmdTimer(void);
    extern L2CAPBuf L2CAP_Buf;
    extern Uint8 CheckValid_L2CAP_Buf(void);
    extern Uint8 (* const Prcss_L2CAP_CID[])(Uint8 connID);
    extern void Rls_L2CAP_Buf(void);
    ...
    if(CheckValid_L2CAP_Buf()==(!0))
    {
        if(LL_CheckConnExist(L2CAP_Buf.connID_LL) == (!0))
        {
            LLHostRun = 0x01;
            Knl_MemCpy(SPIBuffer, L2CAP_Buf.DataBuf, sizeof(L2CAP_Buf.DataBuf));
            if((Prcss_L2CAP_CID[L2CAP_Buf.DataBuf[(0+2)]](L2CAP_Buf.connID_LL)) == 0)    
            {
                if(L2CAP_Buf.DataBuf[(0+2)] == 0x05)
                {
                    if(L2CAP_Buf.DataBuf[(0+4)] == 0x13)
                    {
                        L2cap_update_rsp = (Uint16)((L2CAP_Buf.connID_LL << 8) | L2CAP_Buf.DataBuf[((0+4)+4)]);
...
#pragma Otime
Uint16 EndianSwap16(Uint16 value)
{
    Uint8 i;
...
Uint32 EndianSwap32(Uint32 value)
{
    Uint32 rval;

    ((Uint8 *) &rval)[0] = ((Uint8 *) &value)[3];
    ((Uint8 *) &rval)[1] = ((Uint8 *) &value)[2];
    ((Uint8 *) &rval)[2] = ((Uint8 *) &value)[1];
    ((Uint8 *) &rval)[3] = ((Uint8 *) &value)[0];
    return (rval);
}


void EndianSwap16addr(Uint16 *value)
{
    Uint8 i;
...
void EndianSwap32addr(Uint32 *value)
{
    Uint32 i;

    i = *value;
    ((Uint8 *) value)[0] = ((Uint8 *) &i)[3];
    ((Uint8 *) value)[1] = ((Uint8 *) &i)[2];
    ((Uint8 *) value)[2] = ((Uint8 *) &i)[1];
    ((Uint8 *) value)[3] = ((Uint8 *) &i)[0];
}


void EndianSwap16addrM4(Uint16 *value)
{
    Uint16 i;
...
void EndianSwap16addrM8(Uint16 *value)
{
    Uint16 i;
...
void KernelCollect(void)
{
    Uint8 *a;
    Uint8 *b;

    EndianSwap32addr((Uint32 *)a);
    EndianSwap32((Uint32)a);

    Knl_MemCpyInv_Isr(a, a, 1);
    Knl_CodeCpyInv_Isr(a, b, 1);
    Knl_CodeCpyInv(a, b, 1);
    
    Knl_CodeComp_Isr(a, b, 255);
}
#pragma pop
