...
#line 6 "..\\..\\..\\source\\rffunction.c"

#line 8 "..\\..\\..\\source\\rffunction.c"
...
#line 11 "..\\..\\..\\source\\rffunction.c"
#line 12 "..\\..\\..\\source\\rffunction.c"
...
#line 13 "..\\..\\..\\source\\rffunction.c"
#line 14 "..\\..\\..\\source\\rffunction.c"


extern void Prcss_BLE_CODE_SMP_Pairing_Confirm(void);
...
const Uint8 UUID_BLUETOOTH_BASE[16]=
{
    0x00, 0x00, 0x00, 0x00,     
    0x00, 0x00, 0x10, 0x00,     
    0x80, 0x00, 0x00, 0x80,     
...
CmdTimerMessage CmdTimer[4+1][(0x02+1)];
...
Uint8 setBLE_CmdTimer(Uint8 connID, CmdTimerEvent event, Uint32 timeout_base)
{
    if(CmdTimer[connID][event].Event == 0xFF)
    {
        CmdTimer[connID][event].Event = event;
        CmdTimer[connID][event].Current_T = LLTimeline_Get();
        CmdTimer[connID][event].TimeOut_Base = timeout_base;
...
void checkBLE_CmdTimer(void)
{
    ...
    for(i=0; i<4+1; i++)
    {
        for(j=0; j<(0x02+1); j++)
        {
            if(CmdTimer[i][j].Event != 0xFF)
            {
                if(((LLTimeline_Get() - CmdTimer[i][j].Current_T)&0x0FFFFFFF) > CmdTimer[i][j].TimeOut_Base)
                {
                    switch (CmdTimer[i][j].Event)
                    {
                    case 0x00:
                    {

                        Uint16 L2cap_update_rsp;
                        L2cap_update_rsp = (Uint16)((i << 8) | 0x02);

                        BleMHC_PostEvent(0x05,(void *)&L2cap_update_rsp);

                        CmdTimer[i][j].Event = 0xFF; 
                    }
                    break;
                    ...
                    case 0x01:
                    {
                        ...
                        struct MHC_Le_Conn_Update_Complete_Para event_param;

                        event_param.HCI_Conn_Hdl_L = i;
                        event_param.HCI_Status = 0x02;  

                        BleMHC_PostEvent(0x06,(void *)&event_param);
                    ...
                    case 0x02:
                    {
                        ...
                        struct MHC_Le_Auth_status_Para event_param;
                        event_param.HCI_Conn_Hdl_L = i;
                        event_param.HCI_AUTH_STATUS = 0x0F;  

                        BleMHC_PostEvent(0x0D,(void *)&event_param);
...
void clearBLE_CmdTimer(uint8_t connID, CmdTimerEvent event)
{
    Uint8 i;
    ...
    if(event == 0xFF)
    {
        for(i=0; i<(0x02+1); i++)
        {
            CmdTimer[connID][i].Event = 0xFF; 
        }
    }
    else if(event < (0x02+1))
    {
        CmdTimer[connID][event].Event = 0xFF; 
    }
}
...
uint8_t getBLE_CmdTimerEvent(uint8_t connID, CmdTimerEvent event)
{
    return CmdTimer[connID][event].Event;
}
...
Uint8 setBLE_ConnTxData(Uint8 connID, Uint8 * ScanRspData, Uint8 Length)
{
    
    extern Uint8 LL_Data2ConnTxFIFO(Uint8 connID, Uint8 *Data, Uint8 Length);

    if(LL_Data2ConnTxFIFO(connID, ScanRspData, Length)==(!0))
    {
        return (!0);
    }
    else
...
Uint8 setBLE_ConnTxData_ATT_ERROR_RESPONSE(Uint8 connID, Uint8 Request_Opcode_In_Error, Uint8 ATT_handle_in_error_L, Uint8 ATT_handle_in_error_H, Uint8 Error_code)
{
    Uint8 temp[9];
    ...
    temp[3] = 0x00;                            
    temp[4] = 0X01;       
    temp[5] = Request_Opcode_In_Error;
    temp[6] = ATT_handle_in_error_L;
    temp[7] = ATT_handle_in_error_H;
    temp[8] = Error_code;
    if(setBLE_ConnTxData(connID, temp, (4+5)) == 0)
    {
        return 0;
    }
    else
...
void init_q_att_HDL_PreWr(void)
{
    Uint8 i;
...
void initBLE_AttServParamter(void)
{
    Uint8 i, j, prmt;
    Uint8 size, offset;


    i = (size_ATTRIBUTE_SERVER -1);
    while(i!=0)
    {
        
        
        {
            prmt = *(*(ATTRIBUTE_SERVER_PERMISSION+i)+1);
            if((prmt & 0x03) == 0x01)
            {
                size = *(*(ATTRIBUTE_SERVER+i)+2+2);        
                offset = 3+2;
            }
            else
            {
                size = *(*(ATTRIBUTE_SERVER+i)+16+2);       
                offset = 3+16;
            }
            if((*(ATTRIBUTE_SERVER_PARAM+i)) != (*(ATTRIBUTE_SERVER+i)+(offset)))
            {
                for(j=0; j<size; j++)
                {
                    
                    *((Uint8 *)*(ATTRIBUTE_SERVER_PARAM+i)+j) = *(*(ATTRIBUTE_SERVER+i)+(offset+j));
                ...
                if((prmt & 0x04) == 0x04)
                {
                    ...
                    *((Uint8 *)*(ATTRIBUTE_SERVER_PARAM+i)+j) = *(*(ATTRIBUTE_SERVER+i)+(offset-1));
...
Uint8 chkBLE_ATTRIBUTE_PERMISSION_Read(Uint8 ATT_Handle)
{
    Uint8 i, Rsp_Error;

    i = *(*(ATTRIBUTE_SERVER_PERMISSION+ATT_Handle)+0x00);
    Rsp_Error = 0;
    if((i & (0x01|0x04|0x10|0x40)) == 0)
    {
        Rsp_Error = 0x02;
    }
...
Uint8 chkBLE_ATTRIBUTE_PERMISSION_Write(Uint8 ATT_Handle)
{
    Uint8 i, Rsp_Error;

    i = *(*(ATTRIBUTE_SERVER_PERMISSION+ATT_Handle)+0x00);
    Rsp_Error = 0;
    if((i & (0x02|0x08|0x20|0x80)) == 0)
    {
        Rsp_Error = 0x03;
    }
...
Uint8 getBLE_ATTRIBUTE_TYPE_Size(Uint8 ATT_Handle)
{
    Uint8 i;

    i = (*(*(ATTRIBUTE_SERVER_PERMISSION+ATT_Handle)+0x01) & 0x03);
    if(i == 0x01)
    {
        i = 2;
    }
...
Uint8 getBLE_ATTRIBUTE_Value_DynmcLngth(Uint8 AttHandle, Uint8 SizeAtt)
{

    if((*(*(ATTRIBUTE_SERVER_PERMISSION+AttHandle)+0x01) & 0x04) != 0)
    {
        SizeAtt = ATTRIBUTE_SERVER_PARAM[AttHandle][SizeAtt];
    }
    return (SizeAtt);
}
...
Uint8 chkIfBLE_ATT_Chr_Acss_ByUser(Uint8 AttHandle)
{
    if((*(*(ATTRIBUTE_SERVER_PERMISSION+AttHandle)+0x01) & 0x08) != 0)
    {
        return (!0);
    }
    else
...
Uint8 chkBLE_ATT_Chr_AcssRd_ByUser(Uint8 *AddrAttHandle)
{
    Uint8 Rsp_Error;
...
Uint8 chkBLE_ATT_Chr_AcssWr_ByUser(Uint8 *AddrAttHandle, Uint8 *wrDataBuf)
{
    Uint8 Rsp_Error;
...
Uint8 smp_Authorization;
Uint8 smp_Encryption;
Uint8 smp_Phase;
Uint8 smp_TempDef;
Uint8 smp_Temp16[16];
Uint8 smp_Rand[8];
...
Uint8 smp_SKDm[8];
Uint8 smp_IVm[4];
Uint8 smp_Security_Mode;
Uint8 smp_PReq_IO_Capab;
Uint8 smp_PReq_OOB_data;
Uint8 smp_PReq_Init_Key_Distr;
Uint8 smp_PReq_Resp_Key_Distr;
Uint8 k_AES_passkey[16];
Uint8 r_AES[16];
Uint8 LTK[16];
Uint8 LTK_SEED[8+2];
Uint8 smp_Para_Bond[(36+1)];
Uint8 rsp_pairing_request_table[11];


Uint8 smp_Para_Bond_tmp[70];
Uint8 smp_Local_Addr_advTemp[2+6];
Uint8 smp_Local_Addr[4+1][2+6];
Uint8 SECBuffer[42];
const Uint8 k_AES[16]= {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};   

const Uint8 r_AES_INI[16]= {0x64, 0xe8, 0xb6, 0x77, 0x1b, 0x49, 0x83, 0x23, 0x07, 0x00, 0x4f, 0xf6, 0xc9, 0x16, 0x2e, 0x1d};

const Uint8 RSP_PAIRING_REQUEST[] =
{
    7, 0x00,                                                    
    0x06, 0x00,                                          
    0x02,                                                          
...
const Uint8 IVS_FIXED[] =                                                          
{
    0xDE, 0xAF, 0xBA, 0xBE,                                                             
};


const Uint8 LTK_FIXED[16] =                                                          
{
    0xE2, 0x48, 0x5A, 0x7A,
    0xD8, 0xD9, 0x7C, 0x22,
    0x30, 0x57, 0xFA, 0x42,
...
const Uint8 EDIV_FIXED[2] =                                                          
{
    0x67, 0x75
};

const Uint8 RAND_FIXED[8] =                                                          
{
    0x47, 0x30, 0x4a, 0xf6,
    0xd9, 0x1a, 0x28, 0xdd
};

const Uint8 IRK_FIXED[][16] =
{
    {
        0xB5, 0xB7, 0xE1, 0x48,
        0xAC, 0x7E, 0x3B, 0x91,
...
const Uint8 SEL_STK_GEN_MTHD[5][5] =
{
...
void smpC1(Uint8 * k, Uint8 * r, Uint8 * smpC1_out)
{
    Uint8 i;
    Uint8 p_AES[16];
    
    extern void smpAES_in_PlainText(Uint8 * plaintextData);
    extern void smpAES_out(Uint8 * encDataOut);
    extern const Uint8 TAB_ZERO_128[];

    smpAES_in_key(k);
    Knl_CodeCpyInv_Isr(p_AES, &rsp_pairing_request_table[((0+3)+1)], 7);
    p_AES[7] = smp_PReq_Resp_Key_Distr;
    p_AES[8] = smp_PReq_Init_Key_Distr;
    p_AES[9] = smp_Para_Bond[17];
    p_AES[10] = smp_Para_Bond[36];
    p_AES[11] = smp_PReq_OOB_data;
    p_AES[12] = smp_PReq_IO_Capab;
    p_AES[13] = 0x01;

    p_AES[14] = smp_Local_Addr[4][0];    
    p_AES[15] = smp_Para_Bond[0];   

    for(i=0; i <16; i++)
    {
        p_AES[i] = r[i] ^ p_AES[i];             
    }
    smpAES_in_PlainText(p_AES);
    smpAES_En();

    Knl_CodeCpy_Isr(p_AES, TAB_ZERO_128, 4);
    Knl_MemCpyInv_Isr(&p_AES[4], &smp_Para_Bond[1], 6);
    Knl_MemCpyInv_Isr(&p_AES[10], &smp_Local_Addr[4][2], 6);
    smpAES_out(smpC1_out);
    for(i=0; i <16; i++)
    {
        p_AES[i] = smpC1_out[i] ^ p_AES[i];     
    }
    smpAES_in_PlainText(p_AES);
    smpAES_En();
    smpAES_out(smpC1_out);
...
void smpS1(Uint8 * k, Uint8 * r1, Uint8 * r2, Uint8 * smpS1_out)
{
    Uint8 temp_aes[16];
    extern void smpAES_in_PlainText(Uint8 * plaintextData);
    extern void smpAES_out(Uint8 * encDataOut);

    Knl_MemCpy_Isr(temp_aes, &r1[8], 8);
    Knl_MemCpy_Isr(&temp_aes[8], &r2[8], 8);
    smpAES_in_PlainText(temp_aes);
    smpAES_En();
    smpAES_out(smpS1_out);
}


void smpPasskey_init(void)
{
    Uint8 i;
...
Uint8 smpPasskey_set(Uint32 hexPasskey, Uint8 connID)
{
    Knl_MemCpyInv(&k_AES_passkey[12], (Uint8 *)&hexPasskey, 4);
    if(PassEC_BLE_CODE_SMP(connID, (0x50+0x06), SPIBuffer, 1) == 0) 
    {
        return 0;
    }
    else
...
Uint8 setBLE_ConnTxData_SMP(Uint8 connID, Uint8 L2CAP_Code_SMP, Uint8 * L2CAP_RspDataSMP, Uint8 Length)
{
    extern Uint8 LL_Data2ConnTxFIFO_smp(Uint8 connID, Uint8 *Data, Uint8 Length);
    MBLK Mblk;

    Mblk.Para.HCLL_LE_ACL_Data_Pkt_Para.HCI_DataPkt_Data[0] = (Length+1);       
    Mblk.Para.HCLL_LE_ACL_Data_Pkt_Para.HCI_DataPkt_Data[(0+1)] = 0x00;
    Mblk.Para.HCLL_LE_ACL_Data_Pkt_Para.HCI_DataPkt_Data[(0+2)] = 0x06;
    Mblk.Para.HCLL_LE_ACL_Data_Pkt_Para.HCI_DataPkt_Data[(0+3)] = 0x00;
    Mblk.Para.HCLL_LE_ACL_Data_Pkt_Para.HCI_DataPkt_Data[(0+4)] = L2CAP_Code_SMP;

    
    Knl_MemCpyInv(&Mblk.Para.HCLL_LE_ACL_Data_Pkt_Para.HCI_DataPkt_Data[(0+4)+1], L2CAP_RspDataSMP, Length);

    if(LL_Data2ConnTxFIFO_smp(connID, Mblk.Para.HCLL_LE_ACL_Data_Pkt_Para.HCI_DataPkt_Data, (Length+4+1))==(!0))
    {
        return (!0);
    }
    else
...
#pragma Otime
Uint8 setBLE_ConnTxData_SMP_Isr(Uint8 connID, Uint8 L2CAP_Code_SMP, Uint8 * L2CAP_RspDataSMP, Uint8 Length)
{
    extern Uint8 LL_Data2ConnTxFIFO_smp_Isr(Uint8 connID, Uint8 *Data, Uint8 Length);
    MBLK Mblk;
    ...
    Knl_MemCpyInv_Isr(&Mblk.Para.HCLL_LE_ACL_Data_Pkt_Para.HCI_DataPkt_Data[(0+4)+1], L2CAP_RspDataSMP, Length);

    if(LL_Data2ConnTxFIFO_smp_Isr(connID, Mblk.Para.HCLL_LE_ACL_Data_Pkt_Para.HCI_DataPkt_Data, (Length+4+1))==(!0))
    {
        return (!0);
    }
    else
