...
#line 22 "..\\..\\..\\source\\host.c"
#line 23 "..\\..\\..\\source\\host.c"
...
#line 24 "..\\..\\..\\source\\host.c"
#line 25 "..\\..\\..\\source\\host.c"
...
BLE_Addr_Param bleAddr_param = {0x01,{0x41u, 0x22u, 0x07u, 0x1Du, 0x10u, 0xC3u}};




 
const Uint8 ATT_READ_BY_GROUP_TYPE_REQUEST_BGN[] =
{
    0x01, 0x00,
    0xFF, 0xFF,
    0x00, 0x28,
...
void initBleDeviceParam(BLE_Device_Param *bleDevice)
{
    bleDevice->ble_deviceChipId = 0x66;
    setBLE_BleDeviceAddr(&bleAddr_param);
}


void MHS_Event_Null(MBLK *pMBlk)
{
    
}


void HSPF_Att_Write(MBLK *pMBlk)
{
    extern void (* const ATT_Write[])(uint8_t connID, uint8_t length, uint8_t *srcCMD);

    
    
    (*(ATT_Write[pMBlk->Para.HSPF_Att_Write_Para.HCI_Conn_Hdl_H]))(pMBlk->Para.HSPF_Att_Write_Para.HCI_Conn_Hdl_L, pMBlk->Para.HSPF_Att_Write_Para.HCI_DataPkt_LthL, pMBlk->Para.HSPF_Att_Write_Para.HCI_DataPkt_Data);
}


void HSPF_Att_Read(MBLK *pMBlk)
{
    extern void (* const ATT_Read[])(uint8_t connID, uint8_t *srcCMD);

    (*(ATT_Read[pMBlk->Para.HSPF_Att_Read_Para.HCI_Conn_Hdl_H]))(pMBlk->Para.HSPF_Att_Read_Para.HCI_Conn_Hdl_L, pMBlk->Para.HSPF_Att_Read_Para.HCI_DataPkt_Data);
}


#line 156 "..\\..\\..\\source\\host.c"


void (* const HS_fsm[])(MBLK *) =
{
    MHS_Event_Null,
    HSPF_Att_Write,
    HSPF_Att_Read,
#line 169 "..\\..\\..\\source\\host.c"
};


void Host_Root(Uint8 QueueId)
{
    Uint8 event;
    MBLK *pmblk;
...
BleStackStatus ATT_HDL_Notify(uint8_t connID, uint8_t *addrATT_HDL_INIT, uint8_t *addrATT_HDL_clientCFG, uint8_t *data, uint8_t dataLength)
{
    ...
    if((*addrATT_HDL_clientCFG)&0x01)
    {
        SPIBuffer[(0+4)] = 0X1B;                
        SPIBuffer[0] = (3+dataLength);                                
        SPIBuffer[(0+1)] = 0;                                             
        SPIBuffer[(0+2)] = 0x04;                         
        SPIBuffer[(0+3)] = 0;                                                    
        SPIBuffer[((0+4)+1)] = *(addrATT_HDL_INIT+(0+1));   
        SPIBuffer[((0+4)+2)] = *(addrATT_HDL_INIT+0);   
        
        for(i=0; i<dataLength; i++)   
        {
            SPIBuffer[((0+4)+2)+1+i] = *(data+i);  
        }

        if(setBLE_ConnTxData(connID, SPIBuffer, (dataLength+(3+4))) == 0)    
        {
            return 0x00;
        }
        else
...
BleStackStatus ATT_HDL_Indicate(uint8_t connID, uint8_t *addrATT_HDL_INIT, uint8_t *addrATT_HDL_clientCFG, uint8_t *data, uint8_t dataLength)
{
    ...
    if(ATT_INDICATION_Packet_Sent != 0)
    {
        return 0x01;
    }
    if((*addrATT_HDL_clientCFG)&0x02)
    {
        SPIBuffer[(0+4)] = 0X1D;
        SPIBuffer[0] = (3+dataLength);
        SPIBuffer[(0+1)] = 0;
...
void ATT_HDL_ValueSet(uint8_t *addr_attHDL, uint8_t *data, uint8_t dataLength)
{
    ...
    for(i = 0; i<size_ATTRIBUTE_SERVER; i++)
    {
        if(&(*ATTRIBUTE_SERVER_PARAM[i]) == (&addr_attHDL[0]))
        {
            index = i;
            break;
        }
    ...
    uuidLen = getBLE_ATTRIBUTE_TYPE_Size(index);

    
    maxDataLen = ATTRIBUTE_SERVER[index][(2 + uuidLen)] ;

    
    Knl_MemCpy(addr_attHDL,data,dataLength);

    
    addr_attHDL[maxDataLen] = dataLength;
...
BleStackStatus ATT_HDL_WriteCmd(uint8_t connID, uint16_t handle, uint8_t *data, uint8_t dataLength)
{
    uint8_t i;
    ...
    SPIBuffer[(0+2)] = 0x04;
    SPIBuffer[(0+3)] = 0;
    SPIBuffer[((0+4)+1)] = handle;  
    SPIBuffer[((0+4)+2)] = handle>>8;
...
BleStackStatus ATT_HDL_WriteRequest(uint8_t connID, uint16_t handle, uint8_t *data, uint8_t dataLength)
{
    uint8_t i;
    extern Uint8 ATT_WRITE_REQUEST_Packet_Sent;
    ...
    if(ATT_WRITE_REQUEST_Packet_Sent != 0)
    {
        return 0x01;
    }
    SPIBuffer[(0+4)] = 0X12;
