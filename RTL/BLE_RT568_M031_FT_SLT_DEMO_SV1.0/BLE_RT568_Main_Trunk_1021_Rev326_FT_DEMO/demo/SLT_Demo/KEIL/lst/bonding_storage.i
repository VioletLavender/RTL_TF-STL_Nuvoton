...
#line 1 "..\\..\\SLT_Demo\\BleAppSetting.h"
...
#line 5 "..\\..\\SLT_Demo\\BleAppSetting.h"
...
#line 38 "..\\..\\SLT_Demo\\BleAppSetting.h"
...
#line 22 "..\\..\\..\\source\\bonding_storage.c"
#line 23 "..\\..\\..\\source\\bonding_storage.c"
...
#line 24 "..\\..\\..\\source\\bonding_storage.c"
...
extern const Uint8 INFO_FLASHBOND[(1024/512)][512];
extern const Uint8 DATA_FLASHBOND[(1024/512)][512];
...
uint8_t* cmd_FlashBond(uint8_t opcode, uint8_t * para_data)
{
    ...
    case 0x04:
        *para_data = 0x01;
        for(j=0; j < (1024/512); j++)   
        {
            ...
            {
                k = j;
                if(INFO_FLASHBOND[j][0+x] == (0x00+(k<<3)+(x>>6)))
                {
                    if(INFO_FLASHBOND[j][0+x+1] == INFO_FLASHBOND[j][0+x])
                    {
                        if(INFO_FLASHBOND[j][0+x] != 0xFF)
                        {
                            for(i=(2+x); i<(64+x); i++)
                            {
    ...
    case 0x05:
        *para_data = 0x00;
        ...
        for(j=0; j < (1024/512); j++)   
        {
            for(x=0; x < (512/64); x++)   
            {
                if((INFO_FLASHBOND[((1024/512)-1)-j][(2+(64*((512/64)-1-x)))] != 0xFF))
                {
                    k = INFO_FLASHBOND[(((1024/512)-1)-j)][(2+(64*((512/64)-1-x)))];
                    i = INFO_FLASHBOND[(((1024/512)-1)-j)][(0+(64*((512/64)-1-x)))];
                    break;  
                }
            }
            if(x != (512/64))
            {
                if((k&0x0F) == 0)    
                {
                    if((INFO_FLASHBOND[j][2]) != 0xFF)
                    {
                        if((INFO_FLASHBOND[j][2]) == (k+1))
                        {
                            k = INFO_FLASHBOND[j][2];
                            i = INFO_FLASHBOND[j][0];
                        }
                    }
                }
                break;
    ...
    case 0x01:
        *para_data = 0x00;
        ...
        for(j=0; j < (1024/512); j++)   
        {
            for(x=0; x < (512/64); x++)   
            {
                if(INFO_FLASHBOND[((1024/512)-1)-j][(2+(64*(7-x)))] != 0xFF)
                {
                    *(para_data+2) = INFO_FLASHBOND[(((1024/512)-1)-j)][(2+(64*(7-x)))]+1;
                    *(para_data+1) = INFO_FLASHBOND[(((1024/512)-1)-j)][(2+(64*(7-x)))]+1;
                    if(x==0)
                    {
                        k = 2;
                    }
        ...
        {
            u32Pattern = 0;
            u32Pattern |= *(para_data+2);
            u32Pattern = (u32Pattern << 8) | *(para_data+2);
            u32Pattern = (u32Pattern << 8) | INFO_FLASHBOND[j][k-2];
            u32Pattern = (u32Pattern << 8) | INFO_FLASHBOND[j][k-2];
            flash_program_internal_use(((uint32_t)&INFO_FLASHBOND+((uint16_t)j<<9)+(k-2)), u32Pattern);
        }
        break;
    ...
    case 0x06:
        *para_data = 0x00;
        for(i=0; i<512; i=i+64)    
        {
            if(INFO_FLASHBOND[(*(para_data+1)>>3)][(2+i)] == *(para_data+2))
            {
                for(j=0; j<(36+1); j++)
                {
                    *(para_data+3+j) = INFO_FLASHBOND[(*(para_data+1)>>3)][(4+i+j)];
                }
                break;
            }
        }
    ...
    case 0x07:
        *para_data = 0x00;
        for(i=0; i<(1024/512); i++)      
        {
            for(x=0; x < (512/128); x++)   
            {
                if(DATA_FLASHBOND[((1024/512)-1)-i][2+(128*(((512/128)-1)-x))] == *(para_data+2))
                {
                    break;
                }
            }
        ...
        {
            i = ((1024/512)-1)-i;
            *(para_data+1) = (i*(512/128))+((512/128)-1-x);
            j = 4+1;
            while(j < 128)
            {
                if(DATA_FLASHBOND[i][j+(128*(((512/128)-1)-x))] != 0xFF)
                {
                    k = DATA_FLASHBOND[i][j+(128*(((512/128)-1)-x))];
                    j = j+k+2;
                    if(((k+2)&0x03)!=0)
                    {
                        j = j + (4-((k+2)&0x03));
    ...
    case 0x08:
        *para_data = 0x00;
        for(i=0; i<(1024/512); i++)      
        {
            for(x=0; x < (512/128); x++)   
            {
                if(DATA_FLASHBOND[((1024/512)-1)-i][2 + (128*(((512/128)-1)-x))] != 0xFF)
                {
                    break;
                }
            }
        ...
        *(para_data+1) = (i*(512/128)) + x;

        u32Pattern = 0;
        u32Pattern |= *(para_data+2);
        u32Pattern = (u32Pattern << 8) | *(para_data+2);
        u32Pattern = (u32Pattern << 8) | DATA_FLASHBOND[i][128*x];
        u32Pattern = (u32Pattern << 8) | DATA_FLASHBOND[i][128*x];
        flash_program_internal_use(((uint32_t)&DATA_FLASHBOND+((uint16_t)i<<9)+128*x), u32Pattern);
        *(para_data+3) = 4;
        break;

    case 0x09:
        *para_data = 0x00;
        if(*(para_data+3) >= (128-4))
        {
            *para_data = 0x06;
        }
        else
        {
            for(x=0; x < (512/128); x++)   
            {
                if(DATA_FLASHBOND[(*(para_data+1) >> 2)][2+(128*(((512/128)-1)-x))] != 0xFF)
                {
                    break;
                }
            }
            x = (512/128)-1-x;
            addr = (*(para_data+1) >> 2);
            offsetaddr = *(para_data+3)+(128*x);

            if(((*(para_data+(3+2))+2)&0x03) != 0)
            {
                x = (((*(para_data+(3+2)))+2) >> 2) + 1;
            }
            else
            {
                x = (*(para_data+(3+2))+2) >> 2;
            ...
            for(j=0; j<x; j++)
            {
                Knl_MemCpy((Uint8*)(&u32Pattern),(para_data+((3+1)+i)),4);
    ...
    case 0x0A:
        for(j=0; j <(1024/512); j++)
        {
            for(x=0; x < (512/64); x++)
            {
                if(INFO_FLASHBOND[j][2+(64*x)] == 0xFF)
                {
                    break;
                }
            }
        ...
        if(j != (1024/512))
        {
            ersINFO_FLASHBOND(((1024/512)-1)-j);
            mark_PID_INFO_FLASHBOND(((1024/512)-1)-j);
        }
        for(j=0; j < (1024/512); j++)   
        {
            for(x=0; x < (512/128); x++)   
            {
                if((DATA_FLASHBOND[j][(2+(128*x))] == 0xFF))
                {
                    break;
                }
            }
        ...
        if(j != (1024/512))
        {
            ersDATA_FLASHBOND(((1024/512)-1)-j);
            mark_PID_DATA_FLASHBOND(((1024/512)-1)-j);
        }
        break;
...
void fil_INFO_FLASHBONDwEXST_PBID(void)
{
    ...
    if(*tmp == 0x00)
    {
        tmp = cmd_FlashBond(0x06, (uint8_t*)smp_Para_Bond_tmp);
        if(*tmp == 0x00)
        {
            if(Knl_MemComp((Uint8*)&smp_Para_Bond,(tmp+3),(36+1)))
            {
                for(i=0; i<(512/64); i++)
                {
                    if(INFO_FLASHBOND[(*(tmp+1))>>3][2 + (64*i)] == *(tmp+2))
                    {
                        j = 4 + (64*i);
                        break;
                    }
                ...
                for(j=0; j<program_cnt; j++)
                {
                    ...
                    if(j==(program_cnt-1))
                    {
                        ...
                        {
                            u32FF = 0xFFFFFFFF;  
                            Knl_MemCpyInv((Uint8*)(&u32Pattern)+((36+1)&0x03),(Uint8*)&u32FF,(4-((36+1)&0x03)));
                        }
                    }
...
void fil_DATA_FLASHBONDwEXST_PBID(void)
{
    ...
    if(*tmp == 0x00)
    {
        ...
        if(*tmp != 0x04)
        {
            ...
            while(j < *(tmp+3))
            {
                if(DATA_FLASHBOND[(*(tmp+1))>>2][j+x] != 0xFF)
                {
                    for(i=0; i<(DATA_FLASHBOND[(*(tmp+1))>>2][j+x]); i++)
                    {
                        *((uint8_t*)*(ATTRIBUTE_SERVER_BOND_PARAM+DATA_FLASHBOND[(*(tmp+1))>>2][j+x-1])+i) = DATA_FLASHBOND[(*(tmp+1))>>2][j+x+i+1];
                    }
                    k = DATA_FLASHBOND[(*(tmp+1))>>2][j+x];
                    j = j+k+2;
                    if(((k+2)&0x03) != 0)
                    {
                        j = j + (4 - ((k+2)&0x03));
