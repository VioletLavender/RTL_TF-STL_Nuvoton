...
const Uint8 bleDefaultAddrTable[16][6]  = { {0x11, 0x55,0x44,0x33, 0x22, 0x01}, {0x22, 0x55,0x44,0x33, 0x22, 0x02}, {0x33, 0x55,0x44,0x33, 0x22, 0x03}, {0x44, 0x55,0x44,0x33, 0x22, 0x04}, {0x55, 0x55,0x44,0x33, 0x22, 0x05}, {0x66, 0x55,0x44,0x33, 0x22, 0x06}, {0x77, 0x55,0x44,0x33, 0x22, 0x07}, {0x88, 0x55,0x44,0x33, 0x22, 0x08}, {0x99, 0x55,0x44,0x33, 0x22, 0x09}, {0xAA, 0x55,0x44,0x33, 0x22, 0x0A}, {0xBB, 0x55,0x44,0x33, 0x22, 0x0B}, {0xCC, 0x55,0x44,0x33, 0x22, 0x0C}, {0xDD, 0x55,0x44,0x33, 0x22, 0x0D}, {0xEE, 0x55,0x44,0x33, 0x22, 0x0E}, {0xFF, 0x55,0x44,0x33, 0x22, 0x0F}, {0x01, 0x55,0x44,0x33, 0x22, 0x10}};
...
{
    printf("+====================================================================+\n");
    printf("Press 1 to start SLT test.\n");
    printf("+====================================================================+\n");
}
...
void Ble_Slave_AdvInit(void)
{
    ...
    advParam.Adv_Type = 0x00;
    advParam.Adv_Interval_Min = 32U;
    advParam.Adv_Interval_Max = 32U;
    advParam.Adv_Channel_Map = (0x01 | 0x02 | 0x04);
    advParam.Adv_Filter_Policy = 0x00;
...
void BleApp_Main(void)
{
    ...
    else
    {
        ...
        {
            printf("Result:FAIL  Error Code: 0x%02x\n",status);
        }
...
void RT568_SystemTestInit(void)
{
    ...
    App_SetBleAdvAddress(1);

     
    UART1_SendData((Uint8 *)"SCANENABLE\n",strlen((char *)"SCANENABLE\n"));
}
...
static RT568SLT_TestStatus RT568_SystemTest(void)
{
    ...
    if((appSystemRequest & 0x02 ) != 0)
    {
        appSystemRequest &= ~0x02;

        RT568_SystemTestInit();
        appSystemRequest |= 0x04; 
    }

    
    if((appSystemRequest & 0x04 ) != 0)
    {
        appSystemRequest &= ~0x04;
    ...
    if((appSystemRequest & 0x08 ) != 0)
    {
        appSystemRequest &= ~0x08;
    ...
    if((appSystemRequest & 0x20 ) != 0)
    {
        appSystemRequest &= ~0x20;

        sltTestCase = SLT_TX;
        if(BleDataExcahngeService_SendData(strlen((char *)"SLT-RX-TEST"),(Uint8 *)"SLT-RX-TEST") != 0x00)
        {
            return 0x02;
        }
    ...
    if((appSystemRequest & 0x10 ) != 0)
    {
        appSystemRequest &= ~0x10;
        printf("dongleRssiValue=%d\n",dongleRssiValue);

        sltTestCase = SLT_DUT_RSSI;

        
        if(getBLE_RssiValue(connId) != 0x00)
        {
            return 0x04;
        }
    }
    ...
    if((appSystemRequest & 0x40 ) != 0)
    {
        appSystemRequest &= ~0x40;
        printf("dutRssiValue=%d\n",dutRssiValue);
        ...
        if(setBLE_Disconnect(connId) != 0x00)
        {
            return 0x05;
        }
    }
    ...
    if((appSystemRequest & 0x80 ) != 0)
    {
        appSystemRequest &= ~0x80;
...
static void App_SetBleAdvAddress(uint8_t arg)
{
    ...
    Uint8 bleAddrSetCmd[(6+7)];
    BLE_Addr_Param addrParam;

    
    selectedAddrIndex = (arg & 0x0F);
    addrParam.addrType = 0x00;
    memcpy(addrParam.addr,bleDefaultAddrTable[selectedAddrIndex],6);
    setBLE_BleDeviceAddr(&addrParam);
    printf("selectedAddrIndex = %d \n",selectedAddrIndex);

    
    
    memcpy(bleAddrSetCmd,(Uint8 *)"BTADDR",strlen((char *)"BTADDR"));
    memcpy(bleAddrSetCmd+strlen((char *)"BTADDR"),addrParam.addr,6);
    bleAddrSetCmd[(6+7) - 1] = '\n';
    UART1_SendData(bleAddrSetCmd,(6+7));
}
...
static uint8_t RT568_RssiTest(int8_t dutReadRssi, int8_t dongleReadRssi)
{
    ...
    if((dutReadRssi >= ((-32) + 20)) || (dutReadRssi <= ((-32) - 20)) ||
      ((dongleReadRssi >= ((-32) + 20)) || (dongleReadRssi <= ((-32) - 20))))
    {
...
static BleStackStatus BleDataExcahngeService_SendData(Uint8 length, Uint8 *data)
{
    ...
    if(att_HDL_DATAEXCHANGES_READINIT_MEASUREMENT_CLIENT_CHARACTERISTIC_CONFIGURATION[0] == 0x02)
    {
...
{
    uint8_t txpowerStrLen = strlen((char *)"SLT-TXP-TEST");

    if(memcmp((Uint8 *)"SLT-TX-TEST",data,length) == 0) 
    {
        appSystemRequest |= 0x20;
    }

    if((length == (txpowerStrLen+1)) && ((memcmp((Uint8 *)"SLT-TXP-TEST",data,txpowerStrLen)) == 0)) 
    {
        appSystemRequest |= 0x10;
...
void UART02_IRQHandler(void) __irq
{
    ...
    if ((((((UART_T *) ((( uint32_t)0x40000000) + 0x70000)))->INTSTS & ((0x1ul << (8))))?1:0))
    {
        ...
        while((((((UART_T *) ((( uint32_t)0x40000000) + 0x70000)))->INTSTS & (0x1ul << (0)))>>(0)))
        {
            ...
            uartReceiveByte = ((((UART_T *) ((( uint32_t)0x40000000) + 0x70000)))->DAT);
