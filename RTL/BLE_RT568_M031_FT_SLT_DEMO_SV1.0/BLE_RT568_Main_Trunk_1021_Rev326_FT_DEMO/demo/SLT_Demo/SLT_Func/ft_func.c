...
*   CONSTANT AND DEFINE
*******************************************************************************/
#define ADDED_SLEEPTEST_IN_TXTEST           1
#define ADDED_DEEPSLEEPTEST_IN_TXTEST       1
...
static void UART1_SendCmd(Uint8* data, Uint8 len);
...
RT568FT_TestStatus RT568_SleepTest(void)
{
    ...
    //RF_Tmr_Periodic_initial(4800000,1); // 10 mins
...
RT568FT_TestStatus RT568_RssiTest(int8_t rssiBase, uint8_t rssiRange, uint32_t waitingTime_ms, RssiTestResult* rssiResult)
{
    ...
    Tiny_Delay(10000);
    UART1_SendCmd((Uint8 *)TEST_COMMAND_DUMMY,strlen((char *)TEST_COMMAND_DUMMY));

    /*=================================================================
     *  TX: start TX and wait to get RSSI value from golden dongle
      =================================================================*/
    // RT568 start TX and send command to dongle
    UART1_SendCmd((Uint8 *)TEST_COMMAND_DUT_TXTEST,strlen((char *)TEST_COMMAND_DUT_TXTEST));
...
// D_msg("dongle RX Rssi=%d\n", dongleRssi);
     ...
     *  RX: start to get RSSI value
      =================================================================*/
    RT568_SetRssiTestRx();
    UART1_SendCmd((Uint8 *)TEST_COMMAND_DUT_RXTEST,strlen((char *)TEST_COMMAND_DUT_RXTEST));
...
void dongleMessgae_Handler(uint8_t *data, uint8_t dataLen, void *parms)
{
    ...
    if((dataLen == (strlen((char *)TEST_COMMAND_DUT_TXTEST)+1)) &&
            (strncmp((char *)TEST_COMMAND_DUT_TXTEST,(char *)data,strlen((char *)TEST_COMMAND_DUT_TXTEST)) == 0))
    {
        dongleRssi = (Sint8)data[(strlen((char *)TEST_COMMAND_DUT_TXTEST))];
    }
//  else
//  {
//    D_msg("From UART1:");
