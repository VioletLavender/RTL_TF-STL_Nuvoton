...
void UART13_IRQHandler(void) __irq
{
    static uint8_t uartBuffer[218];
    static uint8_t index = 0u;
    ...
    if (UART_GET_INT_FLAG(UART1,UART_INTSTS_RDAINT_Msk))
    {
        ...
        {
            /* Get the character from UART Buffer */
            uartReceiveByte = UART_READ(UART1);
            uartBuffer[index] = uartReceiveByte;

            if(uartBuffer[index] == '\r' || uartBuffer[index] == '\n')
            {
                dongleMessgae_Handler(uartBuffer,index,NULL);
                index = 0;
            }
            else
            {
