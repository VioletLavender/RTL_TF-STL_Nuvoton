...
{
    printf("+====================================================================+\n");
    printf("Press the number to start related testing.\n");
    printf("+====================================================================+\n");
    printf("1. TX @2402MHz(LDO) Test\n");
    printf("2. TX @2402MHz(DCDC) Test\n");
    printf("3. RX(LDO) Test\n");
    printf("4. RX(DCDC) Test\n");
    printf("5. Sleep Test\n");
    printf("6. Deep Sleep Test\n");
    printf("7. RSSI Test\n");
    printf("8. 16MHz CLK Test\n");
    printf("+====================================================================+\n");
}
...
void RT568_Reset(void)
{
    ...
    PDMA_Close(((PDMA_T *) ((( uint32_t)0x40000000) + 0x08000)));
    SPI_Close(((SPI_T *) ((( uint32_t)0x40000000) + 0x61000)));
...
RT568FT_TestStatus RT568_FTTestSelection(BleFT_TestCase testCase)
{
    ...
    case TX_2402_LDO_TEST:
        ...
        printf("[RT568_FT TX @2402MHz LDO Test]\n");
        break;

    case TX_2402_DCDC_TEST:
        status = RT568_TxTest(DCDC_MODE);
        printf("[RT568_FT TX @2402MHz DCDC Test]\n");
        break;

    case RX_LDO_TEST:
        status = RT568_RxTest(LDO_MODE);
        printf("[RT568_FT RX LDO Test]\n");
        break;

    case RX_DCDC_TEST:
        status = RT568_RxTest(DCDC_MODE);
        printf("[RT568_FT RX DCDC Test]\n");
        break;

    case SLEEP_TEST:
        status = RT568_SleepTest();
        printf("[RT568_FT Sleep Test]\n");
        break;

    case DEEPSLEEP_TEST:
        status = RT568_DeepSleepTest();
        printf("[RT568_FT Deep Sleep Test]\n");
        break;

    case RSSI_TEST:
    {
        RssiTestResult rssiResult;
        status = RT568_RssiTest((-32),20,200, &rssiResult);
        printf("[RT568_FT RSSI Test] DUT RX: %d; Dongle RX: %d\n",rssiResult.dut_rssiValue, rssiResult.dongle_rssiValue);
    }
    break;

    case CLK_16M_TEST:
        status = RT568_16MCLK_Test();
        printf("[RT568_FT 16MHz CLK Test]\n");
        break;
...
void UART13_IRQHandler(void) __irq
{
    ...
    if ((((((UART_T *) ((( uint32_t)0x40000000) + 0x71000)))->INTSTS & ((0x1ul << (8))))?1:0))
    {
        ...
        while((((((UART_T *) ((( uint32_t)0x40000000) + 0x71000)))->INTSTS & (0x1ul << (0)))>>(0)))
        {
            ...
            uartReceiveByte = ((((UART_T *) ((( uint32_t)0x40000000) + 0x71000)))->DAT);
            uartBuffer[index] = uartReceiveByte;

            if(uartBuffer[index] == '\r' || uartBuffer[index] == '\n')
            {
                dongleMessgae_Handler(uartBuffer,index,0);
                index = 0;
            }
            else
            {
